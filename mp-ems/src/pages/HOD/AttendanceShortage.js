...
import {
    ...
    Users, BookOpen, XCircle, TrendingDown,
    RefreshCw, Info, ShieldCheck, Target
} from 'lucide-react';
...
const HODAttendanceShortage = () => {
    ...
    const fetchReport = async () => {
        try {
            ...
            const result = await hodApi.getAttendanceShortage(params);
            setData(result);
            setSearched(true);
    ...
    const handleExport = () => {
        ...
        exportAttendanceCSV(
            data,
            `hod_attendance_shortage_${threshold}pct_${new Date().toISOString().slice(0, 10)}`
        );
    };
        ...
        <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 leading-none">Department Attendance Shortage</h1>
                        <p className="text-sm text-slate-500 mt-1 font-medium">
                            Students in your department below the minimum attendance threshold
                        </p>
                    </div>
                </div>
                <button
                    ...
                    className="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-600 text-white font-bold rounded-xl shadow-lg shadow-violet-200 hover:bg-violet-700 transition-all disabled:opacity-40"
                >
                ...
                <div className="flex flex-wrap gap-5 items-end">
                    ...
                    <div className="min-w-[160px] space-y-2">
                        <label className="text-[12px] font-black text-slate-400 tracking-widest ml-1">
                            Threshold: <span className="text-violet-600">{threshold}%</span>
                        </label>
                        <input
                            type="range"
                    ...
                    <button
                        onClick={fetchReport}
                        disabled={loading}
                        className="h-[45px] px-8 bg-violet-600 text-white font-bold rounded-xl hover:bg-violet-700 transition-all flex items-center gap-2 disabled:opacity-50"
                    >
            ...
            {searched && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    ...
                    <div className="bg-red-50 rounded-2xl border border-red-100 p-6 shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-[12px] font-black text-red-400 tracking-widest">Critical (&lt;60%)</p>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    ...
                                    <th className="px-5 py-4 text-[12px] font-black text-slate-400 tracking-widest text-center">Classes Needed</th>
                                    ...
                                    <tr key={idx} className={`hover:bg-slate-50 transition-colors ${row.status === 'Critical' ? 'bg-red-50/30' : ''}`}>
                                        ...
                                        <td className="px-5 py-3 text-center">
                                            {row.classes_needed > 0 ? (
                                                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-orange-50 border border-orange-200 text-orange-700 text-[11px] font-black">
                                                    <Target size={10} />+{row.classes_needed} classes
                                                </span>
                                            ) : (
                    ...
                    <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center gap-2 text-[12px] text-slate-400 font-bold">
                        <Info size={13} />
                        {data.length} records | {uniqueStudents} unique students below {threshold}% threshold
                    </div>
                </div>
            ) : searched && !loading ? (
                <div className="bg-white rounded-3xl p-20 text-center border border-slate-200">
                    ...
                    <p className="text-slate-500 mt-2">All students in your department meet the {threshold}% attendance threshold.</p>
                </div>
            ) : (
                <div className="bg-white rounded-3xl p-20 text-center border border-slate-200">
...
export default HODAttendanceShortage;
