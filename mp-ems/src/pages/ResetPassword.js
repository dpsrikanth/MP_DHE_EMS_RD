...
import { Lock, Eye, EyeOff, ArrowRight, School, CheckCircle2 } from "lucide-react";
...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await authApi.resetPassword({ resetToken: token, password });
  ...
  return (
    <div className="h-screen bg-white flex flex-col md:flex-row font-sans overflow-hidden">
      {/* Left Panel: Brand */}
      <div className="relative flex-1 bg-indigo-600 overflow-hidden hidden lg:flex flex-col justify-center p-8 xl:p-16">
        <div className="absolute top-0 left-0 w-full h-full opacity-20 pointer-events-none">
          <div className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] bg-indigo- rounded-full blur-[120px]" />
          <div className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-indigo-600 rounded-full blur-[100px]" />
        </div>
        <div className="relative z-10 max-w-xl">
          <div className="flex items-center gap-4 mb-12">
            <div className="w-14 h-14 bg-indigo- rounded-2xl flex items-center justify-center text-white shadow-lg shadow-indigo-500/20">
              <School size={32} />
            </div>
            <div>
              <h1 className="text-3xl font-black text-white tracking-tight italic">Intense</h1>
              <p className="text-indigo-400/80 font-bold text-sm tracking-widest ">Institution Portal</p>
            </div>
          </div>
          <h2 className="text-4xl xl:text-5xl font-black text-white leading-[1.1] mb-6 tracking-tight">
            Secure <span className="text-indigo-400">Reset</span>
          </h2>
          <p className="text-base xl:text-lg text-slate-400 font-medium mb-8 leading-relaxed">
            Please enter your new password to regain access to the portal. Choose a strong, secure password.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-[540px] bg-white flex flex-col justify-center p-6 sm:p-8 lg:p-12 xl:p-16 relative">
        <div className="lg:hidden flex items-center gap-3 mb-8">
          <div className="w-10 h-10 bg-indigo- rounded-xl flex items-center justify-center text-white">
            <School size={24} />
          </div>
        ...
        <div className="max-w-sm mx-auto lg:mx-0 w-full">
          <div className="mb-8 text-center lg:text-left">
            <h2 className="text-2xl xl:text-3xl font-black text-slate-900 mb-2 tracking-tight">Set New Password</h2>
            <p className="text-slate-500 font-medium text-sm xl:text-base">Enter your new secure password below</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <div className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo- transition-colors pointer-events-none">
                  <Lock size={18} />
                </div>
                <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full group relative overflow-hidden bg-indigo-600 text-white rounded-[1.5rem] py-4 font-black text-sm  tracking-[0.15em] shadow-xl shadow-slate-900/10 hover:shadow-indigo-500/20 active:scale-[0.98] transition-all disabled:opacity-70 mt-4"
            >
