...
const Programs = () => {
  ...
  const handleAdd = async () => {
    if (!form.name || !form.duration_years) return alert('Both fields are required');
    try {
      const token = localStorage.getItem('token');
          ...
          <div className="relative bg-white rounded-3xl shadow-3xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 p-8 text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-2">
                Are you sure you want to delete <span className="font-bold text-slate-900 font-mono italic">"{deleteTarget?.name}"</span>? 
