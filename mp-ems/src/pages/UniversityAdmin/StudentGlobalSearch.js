...
const StudentGlobalSearch = () => {
  ...
  const consolidatedRecords = useMemo(() => {
    ...
    studentData.centerHistory.forEach(center => {
      // Find matching semester from marks or just use the center's exam_name to infer
      // For now, we'll try to match by exam_name or just add it to the matching semester name
      // This part might need better backend mapping but let's try a heuristic
    ...
    return Object.values(semesters).sort((a, b) => b.semesterName.localeCompare(a.semesterName));
  }, [studentData]);
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {consolidatedRecords.length > 0 ? (
              <div className="space-y-10">
                {consolidatedRecords.map((record, idx) => (
                  <div key={idx} className="relative">
                    {idx !== consolidatedRecords.length - 1 && (
                      <div className="absolute left-6 top-16 bottom-[-40px] w-0.5 bg-slate-100" />
                    )}
                    
                    <div className="flex items-start gap-6">
                      <div className="w-12 h-12 shrink-0 bg-white border-2 border-indigo-600 rounded-2xl flex items-center justify-center text-indigo-600 shadow-lg z-10 relative">
                        <span className="font-black text-lg">{record.semesterName.match(/\d+/) || 'S'}</span>
                      </div>

                      <div className="flex-1 space-y-4">
                        {/* Semester Header */}
                        <div className="bg-white rounded-2xl p-4 shadow-md border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div>
                            <h4 className="text-lg font-black text-slate-900">{record.semesterName}</h4>
                          ...
                          {record.payments.length > 0 ? (
                            <div className="flex items-center gap-2 bg-emerald-50 px-4 py-1.5 rounded-xl border border-emerald-100">
                              ...
                              <p className="text-[10px] font-black text-emerald-600 uppercase">PAID • ₹{parseFloat(record.payments[0].amount).toLocaleString()}</p>
                            </div>
                          ) : (
                            <div className="flex items-center gap-2 bg-rose-50 px-4 py-1.5 rounded-xl border border-rose-100">
                              <AlertCircle size={14} className="text-rose-500" />
                              <p className="text-[10px] font-black text-rose-600 uppercase">UNPAID / N/A</p>
                            </div>
                          )}
                        </div>
                        ...
                        {record.centers.length > 0 && (
                          <div className="bg-slate-900 rounded-2xl p-4 text-white flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-md overflow-hidden">
                          <table className="w-full text-left">
                            <thead className="bg-slate-50 border-b border-slate-100">
                              <tr>
                                <th className="px-6 py-2 text-[9px] font-black text-slate-400 uppercase">Subject</th>
                                <th className="px-4 py-2 text-[9px] font-black text-slate-400 uppercase text-center">Int</th>
                                <th className="px-4 py-2 text-[9px] font-black text-slate-400 uppercase text-center">Ext</th>
                                <th className="px-4 py-2 text-[9px] font-black text-slate-400 uppercase text-center">Total</th>
                                <th className="px-6 py-2 text-[9px] font-black text-slate-400 uppercase text-center">Status</th>
                              </tr>
                            </thead>
                              ...
                              {record.marks.map((mark, mIdx) => (
                                <tr key={mIdx} className="hover:bg-slate-50/50 transition-colors">
                                  <td className="px-6 py-3">
                                    <p className="text-xs font-black text-slate-800">{mark.subject_name}</p>
                                    <p className="text-[9px] font-bold text-slate-400">{mark.subject_code}</p>
                                  </td>
                                  <td className="px-4 py-3 text-center text-xs font-bold text-slate-500">{mark.internal_marks}</td>
                                  <td className="px-4 py-3 text-center text-xs font-bold text-slate-500">{mark.external_marks}</td>
                                  <td className="px-4 py-3 text-center font-black text-sm">{mark.total_marks}</td>
              ...
              <div className="bg-white rounded-2xl p-8 text-center border border-dashed border-slate-200">
                 <p className="text-slate-400 text-sm font-bold">No semester records found.</p>
              </div>
            )}
          </div>
        </div>
